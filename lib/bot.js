...
function performCommands(msgObj)
{
    ...
    else if (attachment)
    {
        if (attachment[0].type === 'location')
        {
            ...
            console.log(attachment[0].payload.coordinates);
        }
        else
        {
...
{
    sendMessage(sender, "{{user_first_name}} I couldn't understand the input, please give a valid command");
...
function getUserLocation(sender)
{
    var messageData = {
        "text": "And one more thing, please share your location with me so I can do my job!",
        "quick_replies": [
            {
